...
import type { WorkerRequest, WorkerResponse, QueryResult } from './types.ts'
...
function respond(res: WorkerResponse): void { self.postMessage(res) }

function empty(): QueryResult { return { rows: [], changes: 0, lastInsertRowId: 0n } }
...
self.onmessage = ({ data }: MessageEvent<WorkerRequest>) => {
  ...
  try {
    switch (type) {
      case 'open': {
        db = new DatabaseSync(data.path, data.readonly ? { readOnly: true } : undefined)
      ...
      case 'query': {
        ...
        respond({ id, ok: true, data: { rows: rows as Record<string, unknown>[], changes: 0, lastInsertRowId: 0n } })
        break
      }
      case 'execute': {
        if (!db) throw new Error('db not open')
        const r = db.prepare(data.sql).run(..._p(data.params ?? []))
        respond({ id, ok: true, data: { rows: [], changes: Number(r.changes), lastInsertRowId: BigInt(r.lastInsertRowid) } })
        break
      }
      case 'transaction': {
        if (!db) throw new Error('db not open')
        ...
        try {
          for (const { sql, params } of data.stmts) {
            ...
            results.push({ rows: [], changes: Number(r.changes), lastInsertRowId: BigInt(r.lastInsertRowid) })
          }
          db.exec('COMMIT')
        } catch (e) {
          db.exec('ROLLBACK')
