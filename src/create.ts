/**
 * @module
 * aio create — scaffold a new aio project. Runnable straight from JSR:
 *
 * ```sh
 * deno run -A jsr:@riagentic/aio/create my-app
 * ```
 *
 * Also called by `cli.ts` (`aio create <path>`) and `init.ts` (legacy
 * wrapper). Framework delivery: GitHub tarball download (default),
 * `--vendored` git clone, or `--mirror` symlinks (framework-repo dev only).
 */

// deno-lint-ignore no-import-prefix -- create.ts runs standalone via `deno run jsr:...`, so it needs an inline specifier (no import map available)
...
function denoJson(
  ...
  if (appType.hasServer) {
    tasks.am = "deno run -A dep/aio/src/am.ts";
    // Find exposed aio apps on the LAN (UDP via node:dgram — no flags needed).
    tasks.discover = "deno run -A dep/aio/src/am.ts discover";
  }
  // The unified aio client (standalone Electron app for connecting to any
  // remote aio app on the LAN) needs Electron — so compile:client always
  // pairs with install:electron.
  tasks["compile:client"] = "deno run -A dep/aio/src/build.ts --client";
  ...
  const obj: Record<string, unknown> = {
    ...
    version: "0.1.0",
    nodeModulesDir: "auto",
    // "kv" enables Deno.Kv (state persistence). LAN discovery uses node:dgram,
    // which is stable — no "net" / --unstable-net needed.
    unstable: ["kv"],
  };
...
function templateLarge(_title: string): Record<string, string> {
  return {
    "src/app.ts": `import { aio } from 'aio'
...
function applyAppType(
  ...
  // Auth hint for remote server types — --expose is open by default; the
  // config form appends the opt-in inside the object, the zero-arg form gets
  // a commented example. `key: true` = one persisted key (pair the client by
  // PIN); `users` = per-user tokens.
  if (appType.isRemote && appType.hasServer && result["src/app.ts"]) {
    const src = result["src/app.ts"];
    result["src/app.ts"] = /\n\}\)\n$/.test(src)
      ? src.replace(
        /\n\}\)\n$/,
        `\n  // key: true, // --expose auth: a persisted key + PIN pairing\n` +
          `  // users: { 'change-me-token': { id: 'admin', role: 'admin' } }, // or per-user tokens\n})\n`,
      )
      : src.replace(
        /await aio\.run\(([^)]*)\)( ?\/\/[^\n]*)?/,
        `await aio.run($1)$2\n// --expose auth: aio.run({ key: true }) (one key + PIN pairing), ` +
          `or { users: { 'change-me-token': { id: 'admin', role: 'admin' } } }`,
      );
  }
...
/** Scaffold a new aio project — interactive (menus) or fully flagged:
 *  `create(["my-app", "--type=browser", "--template=Minimal"])`.
 *  Delivery: GitHub tarball (default), `--vendored` clone, `--mirror`
 *  symlinks (framework-repo development only). */
...
/** @internal Test-only handle to the template generators. */
export const _test = {
  templateEmpty,
  templateMinimal,
  templateMedium,
