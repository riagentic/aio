...
import type { CellDef, ComposedCells } from "./cell.ts";
import { bindCell } from "./cell.ts";
import { createMemoryMonitor } from "./memory-monitor.ts";
import {
  createAioError,
  reportError as reportAioError,
  type ReportErrorOpts,
} from "./error.ts";
import { AioLogger, log, setLogger } from "./logger.ts";
...
export type BuildLegacyConfigInput = {
  ...
  cellPatchStrategies: Map<
    string,
    import("./state-filter.ts").CellPatchStrategy
  >;
  cellFilterFieldsMap: Map<
    string,
    import("./state-filter.ts").PatchFilterFields
  >;
