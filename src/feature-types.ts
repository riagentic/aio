...
export type Creators = Record<string, (...args: any[]) => Record<string, unknown>>
...
export type Msg<P = unknown> = { type: string; payload: P; _source?: ActionSource }
...
// deno-lint-ignore no-explicit-any
export type FeatureReduceFn = (state: unknown, action: Msg, ctx?: any) => (Msg | ScheduleEffect)[] | void
...
export function checkReservedKeys(featureName: string, keys: string[], kind: string): void {
  for (const key of keys) {
    if (RESERVED_KEYS.has(key)) {
      throw new Error(
        `[feature:${featureName}] ${kind} "${key}" collides with reserved property — ` +
        `"${key}" is used internally by aio. Rename it (e.g. "${key}Data", "${key}Value", "get${key[0]!.toUpperCase()}${key.slice(1)}"). ` +
...
export type FlatActions<A extends Creators> = {
  [K in keyof A & string]: ((...args: Parameters<A[K]>) => { type: string; payload: ReturnType<A[K]> }) & { readonly type: string }
}
...
export type DirectCalling<M> = {
  ...
  [K in keyof M]: M[K] extends (s: any, ...args: infer P) => Promise<infer R>
    ...
    ? ((...args: P) => { type: string; payload: unknown }) & { readonly type: string }
    : never
}
