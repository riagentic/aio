  ...
  return {
    ...
    insertMany(rows: T[]): void {
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        if (rowKeys.length !== keys.length || !keys.every(k => k in (rows[i] as Record<string, unknown>))) {
          ...
          throw new Error(`${name}.insertMany: row ${i} has different columns than row 0 (expected: [${keys.join(', ')}]) — ${parts.join(', ')}`)
        }
      }
      const placeholders = keys.map(() => '?').join(', ')
