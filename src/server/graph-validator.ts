...
export type GraphResult = {
   ...
   *  a crypto wallet blocks `*.server.ts` in the eager set — the dev server 404s those
 ...
 *  is an eager ES link failure — a blank screen every boot (AIO-424, a crypto wallet).
 ...
 *  AIO-427 (a crypto wallet 2026-07-20f): severity is split by CERTAINTY of breakage —
 ...
 *  with (a crypto wallet 2026-07-26: one real fatal buried under 29 old warnings). */
...
export async function validateGraph(
  ...
  // silent white void (a crypto wallet's ask; confirmed on a trading app, whose UI pages import
...
export function extractImportsByKind(
  ...
  // AIO-425 (a news app): a BARE `from "..."` must NOT be matched — JSX text like
  ...
  // graph with a green gate (a crypto wallet, 2026-07-31). Statement-boundary anchored
