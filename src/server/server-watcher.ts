...
export function createFileWatcher(deps: WatcherDeps): FileWatcher {
  ...
  const _warnedCellFiles = new Set<string>(); // trading-app report Bad #2: warn once per cell file
  ...
  function scheduleReload(path: string): void {
    ...
    // trading-app report Bad #2: a changed cell file does NOT hot-reload — cells run in the
      ...
      try {
        ...
        if (/\bcell\s*\(\s*["'`]/.test(src)) {
          if (deps.onCellChange) {
            // Dev restarts the process itself (trading-app report Bad #3) — the handler
  ...
  function startWatcher(): boolean {
    try {
      ...
      // module-errors page blames your code (a crypto wallet 2026-07-26). We can't
