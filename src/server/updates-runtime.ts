...
export function createUpdatesRuntime(deps: UpdatesRuntimeDeps): UpdatesRuntime {
  ...
  async function checkManifest(opts: CheckOptions): Promise<CheckResult> {
    ...
    //
    // WITH THE TRANSPORT, which is the whole guard. `pinKey` refuses a key
    // learned over a transport that does not authenticate the host — and it
    // takes `from` as OPTIONAL, so calling it with two arguments meant the
    // check never ran. `fetchManifest` even computes `pinnable` for this and
    // nothing read it. Over plain http to anything but loopback, whatever the
    // first responder on the path handed over was pinned to
  ...
  async function applyManifest(opts: ApplyOpts): Promise<void> {
    ...
    if (isDirectory) {
      ...
      // …and SHOW IT RUNS. `updates-apply.ts`'s own header states the rule —
      // "nothing is swapped that has not been verified against a trusted key
      // AND shown to actually RUN on this machine" — and `smokeTestArtifact`
      // was called from exactly one place, the single-file swap. The
      // directory ending skipped it, and it is the ending that can least
      // afford to: `judgePendingUpdate` runs INSIDE the new build, so a build
      // that never boots never increments `attempts` and the two-boot
      // auto-rollback never fires. The detached swap script's final
      // `exec "$launch"` has no restore path either. A dead app, marker
      // untouched.
      //
      ...
      if (!smoked.ok) {
        await Deno.remove(staged, { recursive: true }).catch(() => {
          ...
          // refused. Failing to tidy the staged copy leaves a directory
          // behind — `pruneOld` reclaims it — and must not replace the real
          // message with a cleanup error.
        });
