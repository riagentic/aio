...
export interface TrojanDeps {
  ...
  trojan: {
    ...
    /** Recent dispatches + their state diffs (wallet report #4 — `am timeline`). */
    getTimeline?: (
      after?: number,
      limit?: number,
    ) => import("./timeline.ts").TimelineEntry[];
    /** Boot migration + shape-drift picture (wallet report #1 — `am migrations`). */
    getMigrations?: () =>
...
function handleGet(
  ...
  // Recent dispatches + their state diffs (wallet report #4 — `am timeline`). Optional
  ...
  // Boot migration + shape-drift picture (wallet report #1 — `am migrations`). Empty
...
async function handlePost(
  ...
  if (route === "dispatch") {
    try {
      ...
      // a crypto wallet CRITICAL #0: `ok:true` must mean EXECUTED, and an unknown method
