...
export async function setupTransport<S, A>(
  ...
  const dispatchNetwork = (
    ...
    if (cellAccess && cellAccess.size > 0 && type.includes(":")) {
      ...
      if (rule !== undefined) {
        ...
        // (a prediction-market app). Method dispatches carry `payload.args: [...]`.
    ...
    // keystroke pay up to syncIntervalMs of latency (a crypto wallet 2026-07-25:
  ...
  const server: ServerHandle = skipHttp
    ...
    : createServer({
      ...
      // restarts the app instead of asking the developer to (trading-app report Bad #3).
