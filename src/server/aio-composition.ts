...
// leak, so this is escalated from a warning to a boot REFUSAL in dev (a news app Ugly
...
// (wallet report #4: the name heuristic over-fired on exactly these).
 ...
 *  false positives that made the old heuristic cry wolf (wallet report #4). */
...
/** Dev-safety warnings for field-level visibility config (wallet report #1/#2):
...
function warnFieldFilters(composed: ComposedCells): void {
  for (const f of composed.cells) {
    ...
    if (!f.__aio.uiForUser) {
      ...
      // warning about it would penalize the *correct* fix (a crypto wallet). Collect the
      ...
      // — a crypto wallet (2026-07-24 Ugly #7) had to add six `publicFields` one
      ...
      for (const key of topKeys) {
        ...
        // AIO-426 (a news app Ugly #6): an unambiguous credential broadcast to every
