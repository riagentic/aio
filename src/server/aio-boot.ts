...
export interface BootConfig<S> {
  ...
  /** Opt-in durable action journal (wallet report #3) — SIGKILL/power-cut recovery of
...
export interface BootResult<S> {
  ...
  /** Durable action journal (wallet report #3) — null unless `journal: true`. */
  journal: Journal | null;
  /** Boot migration + shape-drift picture (wallet report #1) — undefined when nothing
...
export async function bootStorage<S>(
  ...
  // longer looking at. the wallet found a complete, stale, unguarded wallet
  ...
  // AIO-419 (a crypto wallet): a `db:` table named after a cell's OBJECT slice silently
  ...
  if (shouldPersist && kvDb && hadPersistedState && persistedSnapshot) {
    ...
    const report = cfg.cellMigrations?.size
      ...
      : [];
    // Shape drift (wallet report #1): the RAW stored snapshot vs the declared
  ...
  // Durable journal (wallet report #3) — opt-in, and only where there's a real file to
...
export function detectShapeDrift(
  ...
  const walk = (
    ...
    // whatever `state:` seeded (a crypto wallet 2026-07-28 #2 — a curated token registry
    ...
    if (isPlainObj(decl) && isPlainObj(stor) && depth < DRIFT_MAX_DEPTH) {
      ...
      // (a crypto wallet 2026-07-25 field report: this was a 70-warning wall on boot).
