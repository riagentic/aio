...
async function run(a?: any, b?: any): Promise<AioApp<any, any>> {
  ...
  try {
    ...
    // into the void — no error, dead feature, green tests (a crypto wallet 2026-07-24
...
async function _run<S, A, E>(
  ...
  // Journal recovery (wallet report #3): replay the actions committed AFTER the last
  ...
  //  • the durable journal (wallet report #3) — the crash-recovery tail (actions only),
  //    present only when `journal: true`; sync cells recover via their op-log.
  //  • the in-memory timeline (wallet report #4) — always on, bounded, carries the diff
  ...
  const transport = await setupTransport<S, A>({
    ...
    asyncDb,
    // In-memory dispatch timeline (wallet report #4) — the trojan `timeline` route.
    getTimeline: (after?: number, limit?: number) =>
      timeline.entries(after, limit),
    // Boot migration + shape-drift picture (wallet report #1) — trojan `migrations`.
