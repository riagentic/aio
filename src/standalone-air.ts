...
// toast that returns schedule.after otherwise floods every test (trading-app report Bad #3).
...
// a crypto wallet), we hold `after`/`every` on a virtual clock. `_advanceSchedules(ms)`
...
export function initStandalone<S, A, E>(
  ...
  const dispatch = createDispatch<S, A, E>({
    ...
    execute: (effect) => {
      ...
      // deterministically with ui.advance(ms) / handle.advance(ms) (a crypto wallet).
