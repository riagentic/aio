...
export function generateReplayTest(
  ...
  opts: {
    name?: string;
    cellDir?: string;
    /** Where each cell is DEFINED — `{ spec, binding }` per cell name, with
     *  `spec` the import specifier as seen from the generated file and
     *  `binding` the export that holds it (see {@linkcode findCellDefinitions}).
     *  A cell missing here falls back to the `<cellDir>/<name>.ts` guess, and
     *  the import line SAYS it is a guess. */
    cellFiles?: Record<string, { spec: string; binding: string }>;
  ...
  // The defining FILE, not a guess from the cell's name: `am record` wrote
  // `import { timer } from "../src/timer.ts"` for a cell that lived in
  // src/cell.ts, and `deno task check` was red until it was edited (h3 F8).
  let guessed = 0;
  ...
  return [
    ...
    guessed > 0
      ? `// Adjust the cell import paths marked GUESS and add assertions where marked.`
      : `// Add assertions where marked.`,
    ...(redactedCount > 0
      ? [
        `//`,
        `// ⚠ ${
...
/** Directories a cell definition is never looked for in. */
...
/** Where each named cell is DEFINED under `root`: the file holding
 *  `cell("<name>", …)`, and the exported binding that holds it
 *  (`export const timer = cell("timer", …)` → `timer`). `src/` is searched
 *  first; a cell defined twice keeps the first hit. Pure over the files it
 *  reads; a name with no definition is simply absent from the result.
 *
 *  A regex, like aiol's scan (aiol/context.ts `extractCells`): the goal is an
 *  import line that type-checks, not a parse. */
export function findCellDefinitions(
  root: string,
  names: Iterable<string>,
): Map<string, { file: string; binding: string }> {
  const want = new Set(names);
  const found = new Map<string, { file: string; binding: string }>();
  ...
  const walk = (dir: string, depth: number): void => {
    ...
    try {
      ...
      return; // aio-ok: an unreadable directory holds no definition we can import
    }
    for (const e of entries) {
      ...
      if (e.isDirectory) {
        ...
        /\.(ts|tsx|mts)$/.test(e.name) && !/\.(test|d)\.tsx?$/.test(e.name)
      ) files.push(p);
    }
  };
  ...
  for (const [i, file] of files.entries()) {
    // The root walk re-enters src/ — its files were already read first.
    if (i >= srcCount && file.startsWith(join(root, "src") + "/")) continue;
    ...
    try {
      ...
      continue; // aio-ok: unreadable → not a definition we can point at
    }
    if (!text.includes("cell")) continue;
    const re =
      /(?:export\s+(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*(?::[^=]+)?=\s*)?\bcell\s*(?:<[^>]*>)?\(\s*(['"`])([\w-]+)\2/g;
    for (const m of text.matchAll(re)) {
      const name = m[3]!;
      if (!want.has(name) || found.has(name)) continue;
      // A `cell("x")` in a comment line is an example, not a definition.
      const lineStart = text.lastIndexOf("\n", m.index!) + 1;
      const line = text.slice(lineStart, m.index!).trimStart();
      if (line.startsWith("//") || line.startsWith("*")) continue;
      const binding = m[1] ?? exportedBindingFor(text, m.index!) ?? name;
      found.set(name, { file, binding });
    }
  }
  return found;
}
...
/** Import specifiers for {@linkcode generateReplayTest}, relative to the
 *  directory the generated file will live in. */
export function cellImportSpecs(
  defs: Map<string, { file: string; binding: string }>,
  fromDir: string,
): Record<string, { spec: string; binding: string }> {
  const out: Record<string, { spec: string; binding: string }> = {};
  for (const [name, d] of defs) {
    let spec = relative(fromDir, d.file).replaceAll("\\", "/");
    if (!spec.startsWith(".")) spec = `./${spec}`;
    out[name] = { spec, binding: d.binding };
  }
  return out;
}
...
export async function cmdRecord(
  ...
  // Import each cell from the file that DEFINES it, as seen from where the
  // test is written (stdout: `<project>/tests/`, the conventional home).
  const root = projectRoot();
  const cellNames = new Set(
    actions.map((a) => a.type.split(":")[0]!).filter((c) => c.length > 0),
  );
  const cellFiles = cellImportSpecs(
    findCellDefinitions(root, cellNames),
    outPath ? dirname(resolve(outPath)) : join(root, "tests"),
  );
