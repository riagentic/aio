...
import type {
  Creators, Msg, MachineConfig, ActionUnion,
  ScopedApp, FeatureReduceFn, FeatureExecuteFn, FeatureAio,
  FeatureDef, FlatActions, DirectCalling,
} from './feature-types.ts'
import { RESERVED_KEYS, checkReservedKeys } from './feature-types.ts'
...
export type ReduceHandlers<S, A extends Creators> = Partial<{
...
export type ExecuteHandlers<S, E extends Creators> = Partial<{
...
// deno-lint-ignore no-explicit-any
export function feature<N extends string, S extends Record<string, unknown>, M extends Record<string, Method<S>>>(name: N, config: MethodsFeatureConfig<N, S, M>): FeatureDef<N, any, any, S> & DirectCalling<M>
export function feature<N extends string, S extends Record<string, unknown>, A extends Creators, E extends Creators = Record<string, never>>(name: N, config: ActionsFeatureConfig<N, S, A, E>): FeatureDef<N, A, E, S> & FlatActions<A>
// deno-lint-ignore no-explicit-any
...
function createFeatureFromMethods<N extends string, S extends Record<string, unknown>, M extends Record<string, Method<S>> = Record<string, Method<S>>>(
  ...
  // deno-lint-ignore no-explicit-any
  const explicitReduce = (config as any).reduce as Record<string, (state: unknown, payload: unknown) => void> | undefined
  // deno-lint-ignore no-explicit-any
  const explicitExecute = (config as any).execute as Record<string, (app: ScopedApp, payload: unknown) => void | Promise<void>> | undefined
  ...
  for (const n of generatorNames) {
    if (allNames.has(n)) throw new Error(`[feature:${name}] generator "${n}" collides with ${allNames.get(n)} of same name`)
    allNames.set(n, 'generator')
  }
  for (const n of explicitActionNames) {
    if (allNames.has(n)) throw new Error(`[feature:${name}] action "${n}" collides with ${allNames.get(n)} of same name`)
    allNames.set(n, 'action')
  }
  for (const n of explicitEffectNames) {
    if (allNames.has(n)) throw new Error(`[feature:${name}] effect "${n}" collides with ${allNames.get(n)} of same name`)
  ...
  const reduce: FeatureReduceFn = (state: unknown, action: Msg): (Msg | ScheduleEffect)[] | void => {
    ...
    if (method) {
      if (syncMethods.has(ownKey)) {
        const args = ((action.payload as Record<string, unknown>)?.args as unknown[]) ?? []
    ...
    if (explicitReduce) {
      ...
      if (h) return h(s, (action as { payload: unknown }).payload) as (Msg | ScheduleEffect)[] | void
    }
  }

  // Build executor for async methods
  const execute: FeatureExecuteFn | undefined = asyncMethods.size > 0 || config.effects || explicitExecute
    ? (app: ScopedApp, effect: Msg): void => {
        ...
        if (effect.type === `${prefix}:__exec`) {
          ...
          ;(method as AsyncMethod<S>)(proxy as S, ..._args)
            .then((value) => {
              ...
              if ((app as Record<string, unknown>)._isDisabled && ((app as Record<string, unknown>)._isDisabled as () => boolean)()) {
                resolveCall(_callId, undefined, new Error(`[${name}] feature disabled while ${_method}() was running`))
              } else {
        ...
        const executeHandlers = { ...(typeof config.execute === 'object' ? config.execute : {}), ...(explicitExecute ?? {}) } as Record<string, (app: ScopedApp, payload: unknown) => void | Promise<void>>
  ...
  if (explicitActions) {
    for (const key of explicitActionNames) {
      const label = `${prefix}:${key}`
      publicCatalog[key] = Object.assign(
        (...args: unknown[]) => ({ type: label, payload: explicitActions[key]!(...args) ?? {} }),
        { type: label },
      )
    }
  }
