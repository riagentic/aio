...
export type TestUI = {
   ...
   *  test a server flow that reads a hidden field (a trading platform#3). */
 ...
 *  component genuinely named `Button2` always wins. (a crypto wallet 2026-07-21) */
 ...
 *  to cast was a real papercut (a trading app). The one cast lives here instead. */
// deno-lint-ignore no-explicit-any
...
export function testUI(
  ...
  // dev + prod (a crypto wallet 2026-07-18). Inlined to avoid a cell-test.ts cycle.
...
async function _mountTestUI(
  ...
  // Local hotfix (a prediction-market app): was referenced but never declared — TS2304 +
  // ReferenceError on the auto-DOM path. See dep/aio/feedback/a prediction-market app.md.
  ...
  if (!doc) {
    try {
      ...
      // test with ZERO shim code. Restored on unmount. (a news app R4 P4)
  ...
  // Components using media queries must boot under testUI (a news app audit #7):
  ...
  // localStorage isolation (a trading platform#1). The standalone runtime needs localStorage;
  ...
  // including `ui.exclude`d fields a server route legitimately reads (a trading platform#3).
  ...
  function elementHandle(resolveInfo: () => UIElementInfo): UIElementHandle {
    ...
    // "not a function" TypeError. Assert `ui.….X.disabled` instead. (a news app P1)
  ...
  function lazyHandle(
    ...
    const resolveInfo = (): UIElementInfo => {
      ...
      // shadowed element. (a news app R4 P2)
      ...
      // Point at every live location instead of a dead end. (a crypto wallet)
    ...
    // a bare `TypeError: … is not a function` names nothing. (a news app R4 P1/P2)
  ...
  /** Component/element name SHADOWING (a news app R4 P2): when `name` addresses a
  ...
  function componentHandle(select: () => UISurfaceNode): UIComponentHandle {
    ...
    return new Proxy(base as AnyDoc, {
      get(target, prop: string | symbol) {
        ...
        // (wallet report #2), so `ui.ToolBar.Settings` reaches a `t`-handle inside a
  ...
  const api = {
    ...
    // Unfiltered server-authoritative state (a trading platform#3) — what a server route sees
    ...
    // in tests (a crypto wallet). Then settles so the UI reflects the fired actions.
    ...
    async waitFor(
      ...
      // (`waitFor(pred, "msg")` → TS2559) surprised the field. (a news app R4 P3)
  ...
  return new Proxy(api as AnyDoc, {
    get(target, prop: string | symbol) {
      ...
      if (surf && findComponents(surf, name).length > 0) {
        // Shadow rule (a news app R4 P2): if the name ALSO uniquely addresses an
      ...
      // wallet report #2: hoist a `t`/data-testid element handle to the top level,
