...
/** Arm dev-strict checks for every test harness (a crypto wallet 2026-07-18). Tests
...
> = {
  ...
  /** Assertions */
  expect: {
    /** Assert on cell state slice (typed — `s` is the cell's state, a trading platform B8) */
    // deno-lint-ignore no-explicit-any
 ...
 *  (a trading platform B8 / a news app #8). Selector accessors (plain returns) and actions-form
 ...
 *  resolved return types mirror production `await cell.method(...)` (a trading platform B8 /
 *  a news app #8). Actions-form cells (empty SendSurface) keep the loose senders.
