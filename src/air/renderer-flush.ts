 ...
 * (wallet report #3). Adds an actionable hint when the cause is DOM access with no DOM
    ...
    while (root.pendingComponents.size > 0) {
      ...
      for (let bi = 0; bi < batchItems.length; bi++) {
        ...
        if (_now() >= deadline) {
          ...
          // never surfaced it — the a crypto wallet "rows freeze during an airdrop" class.
