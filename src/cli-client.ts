...
export function connectCliUDS<S>(socketPath: string): CliApp<S> {
  ...
  function connect(): void {
    ...
    Deno.connect({ transport: 'unix', path: socketPath })
      .then(c => {
        ...
        const q = [...queue]
        queue.length = 0
        for (const a of q) writer!.write(encoder.encode(JSON.stringify(a) + '\n')).catch(() => {})
          ...
          try {
            while (true) {
              ...
              for (const line of lines) {
                ...
                if (line.startsWith('__tt:') || line.startsWith('__boot:')) continue
                try {
                  const data = JSON.parse(line)
                  if (data === null || typeof data !== 'object') continue
                  state = data as S
                  if (_readyResolve) { _readyResolve(state); _readyResolve = null }
                  for (const fn of listeners) fn(state)
                } catch { /* bad JSON */ }
              }
            }
  ...
  return {
    get state() { return state },
    get connected() { return conn !== null },
    ready,
