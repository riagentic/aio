 ...
 *  time with a teaching message (a trading app, alpha28 migration): writing
...
export const schedule = {
  ...
  /** Exponential backoff (wallet report #4) — a one-shot `after` whose delay grows
  ...
  /** A self-pacing poller (wallet report #6). Re-issue each cycle with the current
   ...
   *  freeze rendering (wallet report #7). Imperative (returns a Promise), unlike the
...
export function createScheduleManager(
  ...
  function handle(effect: ScheduleEffect): void {
    ...
    // wallet report #5: a dynamic schedule reusing a static id silently replaces it
    if (
