...
export type MethodDraftMeta<S = Record<string, unknown>> = {
  ...
  /** Transactional cells (wallet report #2): publish the buffered write-set atomically
...
// commits over the newer value and nothing says so — a crypto wallet 2026-07-28 #1, where
...
export function createBatcher(
  ...
  // Transactional methods (wallet report #2): buffer every mutation across the whole
...
export function createLiveProxy<S extends Record<string, unknown>>(
  ...
  _signal?: AbortSignal,
  // Transactional mid-method publish (wallet report #2): `s.$commit()` flushes the
  ...
  const handler: ProxyHandler<S> = {
    get(_target, prop, receiver) {
      ...
      if (
        ...
        // (a news app R4). Resolve the element's INDEX instead and hand back the
