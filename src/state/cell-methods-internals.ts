  ...
  return (
    ...
    if (foreignKey) {
      ...
      if (handler) {
        ...
        // (a trading app, alpha28 migration). Non-method triggers pass payload as-is.
...
export function buildMethodsExecutor(
  ...
  // Per-cell serialize mutex (wallet report #2, `transaction: { serialize: true }`):
  ...
  // reporter a shipped data bug (a crypto wallet 2026-07-28 #1): serialize a SYNC method
  ...
  return (app: ScopedApp, effect: Msg): void => {
    ...
    if (effect.type === `${prefix}:__exec`) {
      ...
      // Transactional methods (wallet report #2): reads see a STABLE snapshot captured
      ...
      const runOnce = (): Promise<unknown> => {
        ...
        return (method as AsyncMethod<Record<string, unknown>>)(
          ...
          .finally(() => untrack())
          .then(async (value) => {
            // Transactional commit (wallet report #2): apply the whole method's buffered
          ...
          .catch((e: Error) => {
            // Transactional abort (wallet report #2): a throw/cancel discards the whole
