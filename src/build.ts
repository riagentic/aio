...
async function withDevExcluded(tag: string, fn: (excludes: string[]) => Promise<boolean>): Promise<boolean> {
  ...
  async function _rm(path: string): Promise<void> {
    try { const t = await Deno.readLink(path); saved.push({ path, target: t, isDir: false }); await Deno.remove(path) } catch { /* symlink missing */ }
  }
  async function _rmDir(path: string): Promise<void> {
    try {
      const inner: Array<{ name: string; target: string }> = []
      for await (const e of Deno.readDir(path)) { try { inner.push({ name: e.name, target: await Deno.readLink(join(path, e.name)) }) } catch { /* not a symlink */ } }
...
try {
  for await (const entry of Deno.readDir(dist)) {
    if (entry.name === 'app.js' || entry.name === 'style.css' || entry.name === 'icon.png') continue
