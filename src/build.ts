...
async function withDevExcluded(
  tag: string,
  ...
  try {
    for await (const e of Deno.readDir(_denoDir)) {
      if (e.isDirectory && _devDenoPrefixes.some((p) => e.name.startsWith(p))) {
        excludes.push(join(_denoDir, e.name));
      }
    }
  } catch { /* no .deno dir */ }
...
`;
  ...
  // Copy icon.png to dist/ if it exists (used by UDS electron for panel/taskbar icon)
...
if (doAndroid) {
  ...
  const appNameKotlin = (appTitle ?? binaryName).replace(/[\x00-\x1f\x7f]/g, "")
    .replace(/\\/g, "\\\\").replace(/\$/g, "\\$").replace(/"/g, '\\"');
  const appNameXml = (appTitle ?? binaryName).replace(/&/g, "&amp;").replace(
    /</g,
    "&lt;",
  ).replace(/>/g, "&gt;").replace(/"/g, "&quot;");
...
// deno-lint-ignore no-control-regex
const _stripCtrl = /[\x00-\x1f\x7f\r\n]/g;
const displayName = (appTitle ?? binaryName).replace(_stripCtrl, "");
