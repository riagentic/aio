...
async function setupServer(state: State) {
  ...
  const updateState = (s: State) => { currentState = s }
...
function connectWS(token: string): Promise<{ ws: WebSocket; state: unknown }> {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => { ws.close(); reject(new Error('WS connect timeout')) }, 3000)
    ws.onmessage = (e) => {
...
function nextMessage(ws: WebSocket, ms = 2000): Promise<unknown> {
  return new Promise((resolve, reject) => {
    ...
    const handler = (e: MessageEvent) => {
      ...
      try { resolve(JSON.parse(e.data)) } catch { /* skip */ }
    }
...
Deno.test({ name: 'multi-user: admin sees full state, viewer sees filtered', sanitizeOps: false, sanitizeResources: false }, async () => {
  const state: State = {
    orders: [
...
Deno.test({ name: 'multi-user: concurrent connections get isolated views', sanitizeOps: false, sanitizeResources: false }, async () => {
  const state: State = {
    orders: [
...
Deno.test('multi-user: invalid token rejected', async () => {
  ...
  try {
    ...
    const result = await new Promise<string>((resolve) => {
      ...
      ws.onclose = () => { clearTimeout(timeout); resolve('closed') }
      ws.onerror = () => { clearTimeout(timeout); resolve('error') }
      ws.onmessage = () => { clearTimeout(timeout); resolve('message') }
    })
