...
Deno.test('aiol: flags @std/* import in feature file', async () => {
  await withTmpDir(async (dir) => {
    ...
    await Deno.writeTextFile(join(dir, 'src', 'App.tsx'), 'export default function App() { return <div/> }')
    await Deno.writeTextFile(join(dir, 'deno.json'), JSON.stringify({
      imports: { 'aio': 'jsr:@riagentic/aio@1.0.0' },
      unstable: ['kv'],
    }))
    await Deno.writeTextFile(join(dir, 'src', 'counter.ts'), `
import { join } from '@std/path'
...
`)
    ...
    assertEquals(issues.some(i => i.message.includes('@std/path') && i.message.includes('server-only')), true)
  })
})
...
`)
    const issues = await runCheckUI(dir)
    assertEquals(issues.some(i => i.message.includes('Deno.') && i.message.includes('server-only')), true)
  })
})
...
`)
    const issues = await runCheckUI(dir)
    assertEquals(issues.some(i => i.message.includes('node:fs') && i.message.includes('server-only')), true)
  })
})
...
Deno.test('aiol: flags bare specifier not in deno.json', async () => {
  await withTmpDir(async (dir) => {
    ...
    await Deno.writeTextFile(join(dir, 'src', 'App.tsx'), `
import { useFeature } from 'aio'
import { Terminal } from 'xterm'
export default function App() { return <div/> }
`)
    const issues = await runCheckUI(dir)
    assertEquals(issues.some(i => i.message.includes('xterm') && i.message.includes('deno.json')), true)
  })
})
...
Deno.test('aiol: allows bare specifier that IS in deno.json', async () => {
  await withTmpDir(async (dir) => {
    await Deno.mkdir(join(dir, 'src'), { recursive: true })
    await Deno.writeTextFile(join(dir, 'deno.json'), JSON.stringify({
      imports: { 'aio': 'jsr:@riagentic/aio@1.0.0', 'react': 'npm:react@18', 'xterm': 'npm:xterm@5' },
      unstable: ['kv'],
    }))
    await Deno.writeTextFile(join(dir, 'src', 'App.tsx'), `
import { Terminal } from 'xterm'
...
`)
    await Deno.writeTextFile(join(dir, 'src', 'helpers.ts'), `
import { readFile } from '@std/fs'
export const load = () => readFile('/tmp/x')
`)
    const issues = await runCheckUI(dir)
    assertEquals(issues.some(i => i.severity === 'warn' && i.message.includes('static dynamic import') && i.message.includes('helpers.ts')), true)
  })
})
...
export const counter = feature('counter', { state: { count: 0 }, methods: { inc: (s) => { s.count++ } } })
`)
    await Deno.writeTextFile(join(dir, 'src', 'ui-utils.ts'), `
export function formatNumber(n: number) { return n.toLocaleString() }
`)
    const issues = await runCheckUI(dir)
    assertEquals(issues.some(i => i.message.includes('static dynamic import')), false)
  })
})
...
`)
    ...
    await Deno.writeTextFile(join(dir, 'src', 'features', 'counter.ts'), `
import { feature } from 'aio'
import { loadData } from './helpers.ts'
export const counter = feature('counter', { state: { count: 0 }, methods: { inc: (s) => { s.count++ } } })
`)
    // Helpers has @std import
    await Deno.writeTextFile(join(dir, 'src', 'features', 'helpers.ts'), `
import { join } from '@std/path'
export function loadData() { return join('a', 'b') }
`)
    const issues = await runCheckUI(dir)
    assertEquals(issues.some(i => i.message.includes('@std/path') && i.message.includes('transitive')), true)
  })
})
