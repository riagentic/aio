...
Deno.test("feature: action creators produce { type, payload }", () => {
  ...
  assertEquals(counter.decrement(3), {
...
Deno.test("feature: default params preserved", () => {
  assertEquals(counter.increment(), {
    type: "counter:increment",
    payload: { by: 1 },
  });
});
...
Deno.test("compose: duplicate feature name", () => {
  const a1 = feature("dup", {
    state: {},
    actions: { x: () => ({}) },
    machine: false,
    reduce() {},
  });
  const a2 = feature("dup", {
    state: {},
    actions: { y: () => ({}) },
    machine: false,
    reduce() {},
...
Deno.test("compose: dispatchTo allows dispatching to allowlisted features", () => {
  ...
  const alpha = feature("alpha", {
    ...
    reduce: {
      go() {
        return [alpha.__aio.effects.sync()];
      },
    },
    execute: {
      sync(app) {
...
Deno.test("compose: dispatchTo blocks non-allowlisted features", () => {
  ...
  console.log = (...args: unknown[]) => {
    if (args.join(" ").includes("blocked")) errorLogged = true;
  };
  ...
  const alpha = feature("alpha", {
    ...
    reduce: {
      go() {
        return [alpha.__aio.effects.bad()];
      },
    },
    execute: {
      bad(app) {
...
Deno.test("mixed: methods + actions coexist in one feature", () => {
  ...
  // Method works
  assertEquals(f.increment(), {
    type: "mixed:increment",
    payload: { args: [] },
  });
  assertEquals(f.increment(5), {
    type: "mixed:increment",
    payload: { args: [5] },
  });
  ...
  assertEquals(
    (f as unknown as Record<string, CallableFunction>).SetLabel!("hello"),
...
Deno.test("mixed: methods + actions + effects compose correctly", () => {
  ...
  const markSynced = (f as unknown as Record<string, CallableFunction>)
    .MarkSynced as () => { type: string; payload: unknown };
