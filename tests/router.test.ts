...
Deno.test('matchPath: multiple params', () => {
  assertEquals(matchPath('/users/:userId/posts/:postId', '/users/1/posts/99'), { userId: '1', postId: '99' })
  assertEquals(matchPath('/users/:userId/posts/:postId', '/users/1/posts'), null)
})
...
Deno.test('server: SPA fallback — unknown extensionless path returns HTML', async () => {
  ...
  try {
    ...
    for (const path of ['/users', '/users/42', '/dashboard/settings', '/any/deep/path']) {
      const resp = await fetch(`http://localhost:${SPA_PORT}${path}`)
      assertEquals(resp.status, 200, `${path} should return 200`)
      const body = await resp.text()
      assertEquals(body.includes('<!DOCTYPE html>'), true, `${path} should return HTML`)
    }
