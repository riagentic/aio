...
Deno.test("integration: WS connect → initial state → action → delta broadcast", async () => {
  ...
  const server = createServer({
    ...
    dispatch: (action: unknown) => {
      ...
      if (a.type === "INCREMENT") {
        state = {
          ...state,
          counter: (state.counter as number) + (a.payload?.by ?? 1),
        };
        broadcast?.();
      }
    },
  ...
  try {
    ...
    ws.addEventListener("message", (e) => {
      if (!isProto(e.data as string)) received.push(e.data as string);
    });
...
Deno.test("integration: multiple clients receive broadcasts", async () => {
  ...
  try {
    ...
    ws1.addEventListener("message", (e) => {
      if (!isProto(e.data as string)) msgs1.push(e.data as string);
    });
    ws2.addEventListener("message", (e) => {
      if (!isProto(e.data as string)) msgs2.push(e.data as string);
    });
...
Deno.test("integration: config-level schedule.every dispatches actions on interval", async () => {
  ...
  const dispatch = createDispatch<S, A, E>({
    reduce: (s, a) => {
      if (a.type === "Tick") {
        return { state: { count: s.count + 1 }, effects: [] };
      }
  ...
  assertEquals(
    state.count >= 2,
    true,
    `expected >=2 ticks, got ${state.count}`,
  );
});
    ...
    reduce: (s, a) => {
      if (a.type === "Start") {
        return {
          ...
          effects: [
            schedule.after("save-delay", 50, { type: "Save" }) as unknown as E,
          ],
        };
      }
      if (a.type === "Save") {
        return { state: { ...s, saved: true }, effects: [] };
      }
      return { state: s, effects: [] };
    },
    execute: (effect) => {
