...
const multiSlice = feature('multi', {
  ...
  reduce: {
    ...
    incB(s) { s.b++ },
    incC(s) { s.c++ },
    append(s, p) { if (s.log.length < 1000) s.log.push(p.v) },
  },
})
...
Deno.test('stress: throughput — at least 5k actions/sec', () => {
  ...
  console.log(`  throughput: ${opsPerSec.toLocaleString()} actions/sec (${Math.round(elapsed)}ms for ${N.toLocaleString()})`)

  // Minimum bar — should be well above this on any modern machine
  assertEquals(opsPerSec > 5_000, true, `expected >5k ops/sec, got ${opsPerSec}`)
})
...
Deno.test({ name: 'stress: 100 concurrent generators complete without corruption', sanitizeOps: false, sanitizeResources: false }, async () => {
...
const bigPayload = feature('bigPayload', {
  ...
  reduce: {
    store(s, p) { s.size = p.data.length },
  },
})
