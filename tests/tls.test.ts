...
async function withTempDir(fn: (dir: string) => Promise<void>): Promise<void> {
  ...
  try { await fn(dir) } finally { await Deno.remove(dir, { recursive: true }).catch(() => {}) }
}
...
Deno.test('generated cert is valid PEM format', async () => {
  await withTempDir(async (dir) => {
    ...
    assertEquals(result.cert.trimEnd().endsWith('-----END CERTIFICATE-----'), true)
  })
})
...
Deno.test('missing custom cert file throws', async () => {
  await withTempDir(async (dir) => {
    await assertRejects(
      () => loadOrCreateCert(dir, `${dir}/nonexistent.pem`, `${dir}/also-missing.pem`),
    )
  })
})
