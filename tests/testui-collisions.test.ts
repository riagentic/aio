...
//  a crypto wallet (2026-07-21) — same-type sibling aio-components: only the FIRST
...
//  a news app R4 P2 — component/element name shadowing: `ui.X` used to resolve the
//    COMPONENT, so `.type()`/`.click()` threw. Now the handle is a hybrid —
//    element actions win, component navigation still works underneath.
//  a news app P1 follow-up — interacting with a DISABLED element fails with a clear
//    "is disabled" error (never "not a function"); `.disabled` is assertable.
//  a news app audit 7 — matchMedia is shimmed (real happy-dom impl when owned,
...
Deno.test("siblings: unique t-handles hoist through any instance (ui.Settings)", async () => {
  ...
  const ui = await testUI(makeToolbarApp(clicks));
  try {
    await ui.Settings.click(); //         top-level hoist (wallet report #2)
...
// ── 1b. component/element name shadowing (a news app R4 P2) ───────────────
...
Deno.test("shadow: ui.X acts as the element when a component shadows it", async () => {
  function PasswordInput() {
    ...
    // exact collision from the a news app report.
    return h(
      "div",
      null,
...
// ── 2. disabled elements (a news app P1) ──────────────────────────────────
...
// ── 3. waitFor description string (a news app P3) — timeout carries it ────
...
// ── 4. matchMedia shim (a news app audit 7) ───────────────────────────────
