...
const throwMsg = (fn: () => void): string =>
  (assertThrows(fn) as Error).message;
...
Deno.test("throws: initial state not in declared states", () => {
  ...
  assertEquals(
    m.includes("machine.initial 'missing' not in declared states"),
    true,
  );
});
...
Deno.test("warns but does not throw: dead-end state", () => {
  ...
  log.warn = ((_cat: string, msg: string) => warnings.push(msg)) as any;
  try {
...
Deno.test("throws: multiple errors collected in single throw", () => {
  ...
  assertEquals(
    m.includes("machine.initial 'nope' not in declared states"),
    true,
  );
...
Deno.test("error message: starts with [cell:name] and joins with newlines", () => {
  const m = throwMsg(() =>
  ...
  assertEquals(m.startsWith("[cell:myFeat]"), true);
  assertEquals(m.includes("machine validation failed:"), true);
  assertEquals(m.includes("\n  "), true);
});

Deno.test("error message: cell name is preserved in prefix", () => {
  const m = throwMsg(() =>
  ...
  assertEquals(m.startsWith("[cell:some-long-name]"), true);
});
