...
function makePf(overrides: Partial<PidFile> & { pid: number; port: number }): PidFile {
  return { appId: TEST_APP, startedAt: Date.now(), status: 'started', cwd: Deno.cwd(), ...overrides }
}
...
Deno.test('am: PidFile with status field round-trip', () => {
  const pf = makePf({ pid: 42, port: 9000, startedAt: 1000, status: 'starting' })
  try {
    writePid(pf)
    assertEquals(readPid(TEST_APP), pf)
  } finally { removePid(TEST_APP) }
})
...
Deno.test('am: readPid backward compat — lock without status treated as started', () => {
  try {
    ...
    const raw = JSON.stringify({ appId: TEST_APP, pid: 99, port: 8000, startedAt: 500, cwd: '/tmp' })
...
Deno.test('am-cli: status — exit code 1 when no server', async () => {
  removePid()
  const result = await new Deno.Command('deno', {
    args: ['run', '-A', '--unstable-kv', amScript, '--json', '--port=19899', 'status'],
    stdout: 'piped',
    stderr: 'piped',
  }).output()
  // Exit 1 = stopped
...
Deno.test('am-cli: snapshot load — error on missing file', async () => {
  await withTrojanServer(async () => {
    const r = await runAm(['snapshot', 'load', '/tmp/nonexistent_aio_test_file.json'])
    assertEquals(r.code, 1)
  })
})
...
function runAmOnPort(port: number, args: string[]) {
  return new Deno.Command('deno', {
    args: ['run', '-A', '--unstable-kv', amScript, '--json', `--port=${port}`, ...args],
    stdout: 'piped',
    stderr: 'piped',
  }).output().then(r => {
    const dec = new TextDecoder()
