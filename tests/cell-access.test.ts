...
Deno.test("cellAccessAllowed: rule matrix", () => {
  ...
  // predicate ALSO sees the method's call args → row-level authz (a prediction-market app):
...
Deno.test("cell access: row-level predicate sees method args over the wire (e2e)", async () => {
  ...
  // and compares to the doc's owner. This is the a prediction-market app row-level ask, proven
