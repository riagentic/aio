...
// ── Where operators edge cases ──────────────────────────────────────

Deno.test('sql: where with ne operator', () => {
  const path = tmpDb()
  const schema = { items: table({ id: pk(), name: text(), status: text() }) }
  const { aioDB } = openDb(path, schema)
  const items = tbl<{ id: number; name: string; status: string }>(aioDB, 'items')
  items.insertMany([
    { id: 1, name: 'a', status: 'active' },
    { id: 2, name: 'b', status: 'deleted' },
    { id: 3, name: 'c', status: 'active' },
  ])
  const result = items.where({ status: { ne: 'deleted' } })
  assertEquals(result.length, 2)
  assertEquals(result.every(r => r.status !== 'deleted'), true)
  aioDB.close()
  Deno.removeSync(path)
})

Deno.test('sql: where with like operator', () => {
  const path = tmpDb()
  const schema = { items: table({ id: pk(), name: text() }) }
  const { aioDB } = openDb(path, schema)
  const items = tbl(aioDB, 'items')
  ...
  const result = items.where({ name: { like: 'ali%' } })
  assertEquals(result.length, 2)
  aioDB.close()
  Deno.removeSync(path)
})

Deno.test('sql: where with gt and lt combined', () => {
  const path = tmpDb()
  const schema = { items: table({ id: pk(), value: integer() }) }
  const { aioDB } = openDb(path, schema)
  const items = tbl<{ id: number; value: number }>(aioDB, 'items')
  items.insertMany([
    { id: 1, value: 10 },
    { id: 2, value: 20 },
    { id: 3, value: 30 },
  ...
  const result = items.where({ value: { gt: 10, lt: 40 } })
  assertEquals(result.length, 2)
  assertEquals(result.map(r => r.value).sort(), [20, 30])
  aioDB.close()
  Deno.removeSync(path)
})

Deno.test('sql: where with gte and lte', () => {
  const path = tmpDb()
  const schema = { items: table({ id: pk(), value: integer() }) }
  const { aioDB } = openDb(path, schema)
  const items = tbl(aioDB, 'items')
  ...
  const result = items.where({ value: { gte: 10, lte: 20 } })
  assertEquals(result.length, 2)
  aioDB.close()
  Deno.removeSync(path)
})

Deno.test('sql: where with in operator', () => {
  const path = tmpDb()
  const schema = { items: table({ id: pk(), name: text() }) }
  const { aioDB } = openDb(path, schema)
  const items = tbl(aioDB, 'items')
  ...
  const result = items.where({ name: { in: ['a', 'c'] } })
  assertEquals(result.length, 2)
  aioDB.close()
  Deno.removeSync(path)
})

Deno.test('sql: where with empty in array matches nothing', () => {
  const path = tmpDb()
  const schema = { items: table({ id: pk(), name: text() }) }
  const { aioDB } = openDb(path, schema)
  const items = tbl(aioDB, 'items')
...
Deno.test('sql: where null value uses IS NULL', () => {
  const path = tmpDb()
  const schema = { items: table({ id: pk(), name: text({ nullable: true }) }) }
  const { aioDB } = openDb(path, schema)
  aioDB.run('INSERT INTO items (id, name) VALUES (?, ?)', [1, null])
  aioDB.run('INSERT INTO items (id, name) VALUES (?, ?)', [2, 'hello'])
  const items = tbl<{ id: number; name: string | null }>(aioDB, 'items')
  const result = items.where({ name: null })
  assertEquals(result.length, 1)
...
Deno.test('sql: insertMany with inconsistent columns gives helpful error', () => {
  const path = tmpDb()
  const schema = { items: table({ id: pk(), name: text(), value: integer() }) }
  const { aioDB } = openDb(path, schema)
  const items = tbl(aioDB, 'items')
  assertThrows(
    () => items.insertMany([
      { id: 1, name: 'a', value: 10 },
      { id: 2, name: 'b' },  // missing value
    ] as any),
    Error,
    'different columns',
  )
...
Deno.test('sql: count returns correct number', () => {
  const path = tmpDb()
  const schema = { items: table({ id: pk(), name: text() }) }
  const { aioDB } = openDb(path, schema)
  const items = tbl(aioDB, 'items')
  ...
  assertEquals(items.count(), 3)
  assertEquals(items.count({ name: 'a' }), 1)
  aioDB.close()
  Deno.removeSync(path)
})

Deno.test('sql: where with limit and offset', () => {
  const path = tmpDb()
  const schema = { items: table({ id: pk(), name: text() }) }
  const { aioDB } = openDb(path, schema)
  const items = tbl<{ id: number; name: string }>(aioDB, 'items')
  items.insertMany([
    { id: 1, name: 'a' },
    { id: 2, name: 'b' },
    { id: 3, name: 'c' },
  ...
  const result = items.where({}, { limit: 2, offset: 1 })
  assertEquals(result.length, 2)
