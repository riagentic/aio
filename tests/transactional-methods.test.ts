// wallet report #2 — transactional cell methods (opt-in `transaction: true`). Reads see
...
Deno.test("transaction serialize: a guard on a field a SYNC method writes is not inert", async () => {
  // a crypto wallet 2026-07-28 #1, verbatim: refresh() fetches for a while; a send calls
