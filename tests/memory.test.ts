...
Deno.test('memory: 10k dispatches — heap growth < 20MB', async () => {
  ...
  console.log(`  heap growth: ${growthMB.toFixed(2)} MB after ${N.toLocaleString()} dispatches`)

  // 20MB is generous — typical growth is < 5MB for pure reduce
  assertEquals(growthMB < 20, true, `heap grew ${growthMB.toFixed(1)}MB — possible leak`)
})

Deno.test({ name: 'memory: 100 generator cycles — no listener leak', sanitizeOps: false, sanitizeResources: false }, async () => {
  resetFlows()
  ...
  assertEquals(s.completed >= 1, true, `expected >=1 completed, got ${s.completed}`)
})

Deno.test({ name: 'memory: waitFor listeners cleaned up after signal', sanitizeOps: false, sanitizeResources: false }, async () => {
  resetFlows()
...
Deno.test({ name: 'memory: waitFor listeners cleaned up on timeout', sanitizeOps: false, sanitizeResources: false }, async () => {
  resetFlows()
...
Deno.test('memory: rapid state reset prevents unbounded growth', async () => {
  ...
  console.log(`  heap growth after 100 fill/clear cycles: ${growthMB.toFixed(2)} MB`)

  // If arrays leak, 100 × 1000 items would accumulate. Should be near zero.
  assertEquals(growthMB < 20, true, `heap grew ${growthMB.toFixed(1)}MB — old state not GC'd`)
