// a crypto wallet 2026-07-24 #3 — `aio doctor` integrity sweep: structural problems no
...
Deno.test("doctor: a server-only import in a browser-bundle cell file fails the sweep (wallet report #1d)", async () => {
  const dir = await project({
