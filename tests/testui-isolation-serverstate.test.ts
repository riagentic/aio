...
//  • a trading platform#1 — the in-memory localStorage shim is now installed FRESH per mount
//    and torn down, so writes don't bleed test→test (it used to be a
//    never-reset process global).
//  • a trading platform#3 — ui.serverState()/ui.fullState() expose the UNFILTERED server
...
Deno.test("testUI: localStorage does not bleed across mounts (a trading platform#1)", async () => {
  {
...
Deno.test("testUI: serverState()/fullState() expose ui.exclude'd fields (a trading platform#3)", async () => {
  ...
  // nothing (a crypto wallet 2026-07-28 #3).
  let threw = "";
  try {
