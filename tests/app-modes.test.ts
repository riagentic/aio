...
function createApp(initial: Record<string, unknown>) {
  ...
  return {
    ...
    dispatch: (action: unknown) => {
      ...
      if (a.type === 'INC') state = { ...state, count: (state.count as number) + 1 }
      _broadcast?.()
    },
    setBroadcast: (fn: () => void) => { _broadcast = fn },
  }
}
...
Deno.test('mode: browser local — serve HTML + WS state sync', async () => {
  ...
  const server = createServer({
    port: P + 1, title: 'Browser Local',
    getUIState: () => app.getState(), dispatch: (a) => app.dispatch(a),
    baseDir: dir, debug: () => {}, prod: true, distDir: join(dir, 'dist'),
  })
  ...
  try {
    ...
    ws.onmessage = (e) => { if (typeof e.data === 'string') messages.push(e.data) }
...
Deno.test('mode: electron local — UDS transport + state sync', async () => {
  const dir = await makeTempBase()
  const socketPath = join(dir, 'electron.sock')
  const app = createApp({ count: 0, label: 'electron-local' })

  const uds = createUDSListener(socketPath, () => app.getState(), (a) => app.dispatch(a), () => {})
...
Deno.test('mode: cli local — headless server + connectCli state sync', async () => {
  ...
  try {
    const cli = connectCli<{ count: number; label: string }>(`http://127.0.0.1:${P + 3}`)
    const state = await cli.ready
    assertEquals(state.count, 0)
...
Deno.test('mode: service local — headless server + trojan API', async () => {
  ...
  const server = createServer({
    port: P + 4, title: 'Service Local',
    getUIState: () => app.getState(), dispatch: (a) => app.dispatch(a),
    baseDir: dir, debug: () => {}, prod: true, distDir: join(dir, 'dist'),
    trojan: { getState: () => app.getState(), getSchedules: () => [], startedAt: Date.now() - 5000 },
  })
  app.setBroadcast(server.broadcast)
  await new Promise(r => setTimeout(r, 50))

  try {
    const stateResp = await fetch(`http://127.0.0.1:${P + 4}/__aio/trojan/state`)
    assertEquals(stateResp.status, 200)
    const state = await stateResp.json()
    assertEquals(state.count, 10)

    const dispatchResp = await fetch(`http://127.0.0.1:${P + 4}/__aio/trojan/dispatch`, {
    ...
    const configResp = await fetch(`http://127.0.0.1:${P + 4}/__aio/trojan/config`)
    assertEquals(configResp.status, 200)
    const config = await configResp.json()
    assertEquals(config.port, P + 4)
    assertEquals(config.title, 'Service Local')

    const metricsResp = await fetch(`http://127.0.0.1:${P + 4}/__aio/trojan/metrics`)
...
Deno.test('mode: electron remote — expose + token, thin client via WS', async () => {
  ...
  const server = createServer({
    port: P + 6, title: 'Electron Remote',
    getUIState: () => app.getState(), dispatch: (a) => app.dispatch(a),
    baseDir: dir, debug: () => {}, prod: true, distDir: join(dir, 'dist'),
    expose: true, token,
  })
  ...
  try {
    const cli = connectCli<{ count: number; label: string }>(`http://127.0.0.1:${P + 6}`, { token })
    const state = await cli.ready
...
Deno.test('mode: cli remote — expose + token auth + connectCli', async () => {
  ...
  try {
    const cli = connectCli<{ count: number; label: string }>(`http://127.0.0.1:${P + 7}`, { token })
    const state = await cli.ready
    assertEquals(state.count, 0)
...
Deno.test('mode: service remote — expose + token + trojan API', async () => {
  ...
  const server = createServer({
    port: P + 8, title: 'Service Remote',
    getUIState: () => app.getState(), dispatch: (a) => app.dispatch(a),
    baseDir: dir, debug: () => {}, prod: true, distDir: join(dir, 'dist'),
    expose: true, token,
    trojan: { getState: () => app.getState(), getSchedules: () => ['heartbeat'], startedAt: Date.now() - 60_000 },
  })
  app.setBroadcast(server.broadcast)
  await new Promise(r => setTimeout(r, 50))
  ...
  try {
    ...
    const stateResp = await fetch(`http://127.0.0.1:${P + 8}/__aio/trojan/state?token=${token}`)
    assertEquals(stateResp.status, 200)
    const state = await stateResp.json()
    ...
    const dispatchResp = await fetch(`http://127.0.0.1:${P + 8}/__aio/trojan/dispatch?token=${token}`, {
      method: 'POST', headers: { 'Content-Type': 'application/json', 'X-AIO': '1' },
      body: JSON.stringify({ type: 'INC' }),
    })
    assertEquals(dispatchResp.status, 200)
    ...
    const schedResp = await fetch(`http://127.0.0.1:${P + 8}/__aio/trojan/schedules?token=${token}`)
    ...
    const cli = connectCli<{ count: number }>(`http://127.0.0.1:${P + 8}`, { token })
...
Deno.test('mode: multi-user — different users see different state', async () => {
  const dir = await makeTempBase()
  const app = createApp({ count: 100, secret: 'admin-only', label: 'multi-user' })
  ...
  try {
    ...
    const adminCli = connectCli<{ count: number; secret?: string }>(`http://127.0.0.1:${P + 9}`, { token: 'admin-tok' })
    ...
    const viewerCli = connectCli<{ count: number; secret?: string }>(`http://127.0.0.1:${P + 9}`, { token: 'viewer-tok' })
...
Deno.test({ name: 'mode: full aio.run with feature — boot + dispatch + shutdown', sanitizeOps: false, sanitizeResources: false, fn: async () => {
  ...
  const counter = feature('counter', {
    ...
    methods: {
      ...
      decrement(s: { count: number }, by = 1) { s.count -= by },
      reset(s: { count: number }) { s.count = 0 },
    },
  })
