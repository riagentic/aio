...
Deno.test("validateGraph: a node: import in the client graph BLOCKS (sandboxed renderer can't load it)", async () => {
  ...
  try {
    ...
    // silent blank screen (a crypto wallet's ask; confirmed against a trading app).
    assertEquals(result.valid, false);
...
Deno.test("validateGraph: a server-only module reached ONLY via dynamic import does NOT block (the escape hatch works — a trading app)", async () => {
  const dir = await Deno.makeTempDir();
  try {
...
// AIO-425 (a news app): a bare `from "` inside a STRING LITERAL (JSX text) must NOT be
...
// a crypto wallet 2026-07-26: `await import("aio/server")` inside a cell method — the
