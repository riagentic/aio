...
Deno.test("monogram: the first character that HAS a glyph", () => {
  ...
  assertEquals(monogramChar("-a trading app"), "Q");
});
...
Deno.test("svg: self-contained, font-free, correctly sized", () => {
  const svg = appIconSvg("a trading app", 128);
...
Deno.test("png: a real PNG whose glyph actually covers pixels", async () => {
  const png = await appIconPng("a trading app", 64);
  ...
  const px = appIconPixels("a trading app", 64);
  const mid = 32;
  let ink = 0;
  for (let x = 0; x < 64; x++) {
    const o = (mid * 64 + x) * 4;
