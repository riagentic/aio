...
import { _reset, initStandalone } from "../src/standalone.ts";
...
Deno.test("dispatch handles re-entrant dispatches", async () => {
  ...
  const reduce = (
    ...
    if (action.type === "DOUBLE") {
      return {
        ...
        effects: [{ type: "CASCADE", payload: { message: "" } }],
      };
    }
  ...
  const execute = (
    _app: ReturnType<typeof initStandalone<State, Action, Effect>>,
    effect: Effect,
  ) => {
...
Deno.test("localStorage restore preserves new schema keys", async () => {
  ...
  const app = initStandalone(
    { count: 0, name: "default" } as State & { name: string },
    {
...
Deno.test("invalid reducer output is handled gracefully", async () => {
  setup();
  const badReduce = (_s: State, _a: Action) =>
    "oops" as unknown as { state: State; effects: Effect[] };
  const app = initStandalone({ count: 0 }, {
    reduce: badReduce,
    execute: () => {},
  });
...
Deno.test("invalid effects are skipped", async () => {
  ...
  const reduce = (_s: State, _a: Action) => ({
    state: { count: 1 },
    effects: [null as unknown as Effect, {
      type: "GOOD",
      payload: { message: "ok" },
    }],
  });
...
Deno.test("ScheduleEffect from reduce is not passed to execute", async () => {
  ...
  const reduce = (
    state: State,
    action: Action,
  ): { state: State; effects: (Effect | ScheduleEffect)[] } => {
...
Deno.test("close then re-init: clean slate", async () => {
  setup();
  const app1 = initStandalone({ count: 0 }, {
    reduce: makeReduce(),
    execute: () => {},
    persist: false,
  });
  ...
  _reset();
  const app2 = initStandalone({ count: 0 }, {
    reduce: makeReduce(),
    execute: () => {},
    persist: false,
  });
