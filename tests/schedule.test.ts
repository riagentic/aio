...
Deno.test("schedule.backoff: exponential growth, capped at max (wallet report #4)", () => {
  const A = { type: "poll" };
...
Deno.test("schedule.poll: constant while healthy, backs off on failure (wallet report #6)", () => {
  const A = { type: "tick" };
