...
Deno.test('manager: invalid schedule id throws', () => {
  const mgr = createScheduleManager(() => {}, noop)
  assertThrows(() => mgr.handle(schedule.after('bad id!', 100, { type: 'X' })), Error, 'invalid schedule id')
  assertThrows(() => mgr.handle(schedule.after('', 100, { type: 'X' })), Error, 'invalid schedule id')
})

Deno.test('manager: start with cron schedule creates active timer', () => {
  const mgr = createScheduleManager(() => {}, noop)
  mgr.start([{ id: 'cron-job', cron: '0 * * * *', action: { type: 'Hourly' } }])
