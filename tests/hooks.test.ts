...
Deno.test("hooks: onAction fires before reduce", () => {
  ...
  const onAction = (a: { type: string }, _s: typeof state) => {
    log.push(`hook:${a.type}`);
  };
...
Deno.test("hooks: onEffect fires before execute", () => {
  ...
  const onEffect = (e: { type: string }) => {
    log.push(`hook:${e.type}`);
  };
  const execute = (e: { type: string }) => {
    log.push(`exec:${e.type}`);
  };
...
Deno.test("hooks: onEffect error does not prevent execute", () => {
  ...
  const hookedExecute = (e: { type: string }) => {
    try {
      throw new Error("effect hook boom");
    } catch { /* guarded */ }
...
Deno.test("hooks: onConnect error does not break WS", async () => {
  ...
  try {
    ...
    ws.addEventListener("message", (e) => {
      ...
      if (d.includes('"t":"state"')) msgs.push(d);
    });
    await new Promise<void>((r) => {
      ws.onopen = () => r();
    });
    ...
    assertEquals(JSON.parse(msgs[0]!).d.ok, true);
...
Deno.test("hooks: onConnect receives AioUser when users configured", async () => {
  ...
  try {
    const ws = new WebSocket(
      `ws://127.0.0.1:${HOOK_PORT}/ws?token=test-token-abc`,
    );
    await new Promise<void>((r) => {
      ws.onopen = () => r();
    });
...
Deno.test("hooks: onRestore transforms state", () => {
  ...
  const merged = {
    items: [{ name: "a" }, { name: "b", score: undefined }],
    count: 5,
  };
...
Deno.test("hooks: onRestore error is survivable (error-guarded pattern)", () => {
  const state = { n: 42 };
  const onRestore = (_s: typeof state): typeof state => {
    throw new Error("restore boom");
  };
  // Simulates the try/catch guard in aio.ts
  let result = state;
  try {
    result = onRestore(state);
  } catch { /* guarded — keep original state */ }
