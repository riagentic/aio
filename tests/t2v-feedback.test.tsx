// A field report from a video tool (a Deno/Electron app: two prompt stages, an embedded
...
const shot = cell("a video tool-shot", {
...
const neg = cell("a video tool-neg", {
  state: { on: false },
  methods: {
    toggle(s: { on: boolean }) {
      s.on = !s.on;
...
const stage = cell("a video tool-stage", {
  state: { stage: "a" },
  methods: {
...
const rows = cell("a video tool-rows", {
...
const studio = cell("a video tool-studio", {
...
testUI(
  ...
  { seed: { "a video tool-studio": { view: "advanced" } } },
  async (ui) => {
    assert(ui.html().includes("IMAGE-STAGE"), ui.html());
    await ui.vid.click();
    await ui.settle();
