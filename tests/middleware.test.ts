...
Deno.test('composeMiddleware: user is passed to middleware functions', () => {
  ...
  // admin can dispatch
  assertEquals(composed({ type: 'admin:delete' }, state, admin), { type: 'admin:delete' })
  // anon blocked
  assertEquals(composed({ type: 'admin:delete' }, state, undefined), null)
  // non-admin blocked
  assertEquals(composed({ type: 'admin:delete' }, state, { id: 'u2', role: 'viewer' }), null)
  // non-admin actions pass through
  assertEquals(composed({ type: 'counter:increment' }, state, { id: 'u2', role: 'viewer' }), { type: 'counter:increment' })
})