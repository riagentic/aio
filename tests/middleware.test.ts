...
Deno.test("composeMiddleware: passes action through single function", () => {
  ...
  const enrich = (a: Action, _s: State): Action => ({
    ...a,
...
Deno.test("composeMiddleware: composes multiple functions in order", () => {
  const state: State = { count: 0 };
  const addTimestamp = (a: Action, _s: State): Action => ({
    ...a,
    payload: { ts: 123 },
  });
  const addVersion = (a: Action, _s: State): Action => ({
    ...a,
    payload: { ...a.payload as Record<string, unknown>, version: 1 },
  });
...
Deno.test("composeMiddleware: user is passed to middleware functions", () => {
  const state: State = { count: 0 };
  const admin: AioUser = { id: "u1", role: "admin" };
  const authorize = (a: Action, _s: State, user?: AioUser): Action | null => {
    if (a.type === "admin:delete" && user?.role !== "admin") return null;
    return a;
  };
  const composed = composeMiddleware(authorize);

  // admin can dispatch
  assertEquals(composed({ type: "admin:delete" }, state, admin), {
    type: "admin:delete",
  });
  // anon blocked
  assertEquals(composed({ type: "admin:delete" }, state, undefined), null);
  // non-admin blocked
  assertEquals(
    composed({ type: "admin:delete" }, state, { id: "u2", role: "viewer" }),
    null,
  );
  // non-admin actions pass through
  assertEquals(
    composed({ type: "counter:increment" }, state, {
      id: "u2",
      role: "viewer",
    }),
