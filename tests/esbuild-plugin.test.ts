...
type ResolveCallback = (args: { path: string }) => { path: string; namespace: string } | undefined
type LoadCallback = (args: { path: string; namespace: string }) => { contents: string; loader: string } | undefined

function collectPluginCallbacks(plugin: { name: string; setup: (build: unknown) => void }) {
  const resolvers: { filter: RegExp; cb: ResolveCallback }[] = []
  const loaders: { filter: RegExp; namespace: string; cb: LoadCallback }[] = []
  const build = {
    onResolve(opts: { filter: RegExp }, cb: ResolveCallback) { resolvers.push({ filter: opts.filter, cb }) },
    onLoad(opts: { filter: RegExp; namespace: string }, cb: LoadCallback) { loaders.push({ filter: opts.filter, namespace: opts.namespace, cb }) },
  }
  plugin.setup(build)
...
Deno.test('plugin: proxy module includes package name in error', () => {
  ...
  const { loaders } = collectPluginCallbacks(plugin)
  const loader = loaders.find(l => l.namespace === 'aio-server-only')!
  const result = loader.cb({ path: '@std/path', namespace: 'aio-server-only' })!
