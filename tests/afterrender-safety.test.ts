// afterRender safety (wallet report #3): a lifecycle hook that reaches for global
...
Deno.test("afterRender: a throwing hook does not collapse the surface", async () => {
  ...
  function Panel() {
    // Simulate a crypto wallet's bug: reach for a global that isn't there in testUI.
    onMount(() => {
