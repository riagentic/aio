...
export const heavy = cell("heavy", {
  ...
  methods: {
    ...
    // call ceiling, success and failure alike (a crypto wallet, 2026-08-01).
    ...
    // Reads a PEER cell from inside the worker — the trap a crypto wallet hit: this used
...
Deno.test({
  ...
  ignore: Deno.build.os === "windows",
  async fn() {
    // The regression this pins (a crypto wallet, 2026-08-01): an async worker method's
...
Deno.test({
  ...
  ignore: Deno.build.os === "windows",
  async fn() {
    // Field report (a crypto wallet, 2026-07-26): boot validation catches config-level
