...
Deno.test("visibility #4: public/id fields do NOT trip the secret heuristic", () => {
  // wallet report #4: the name heuristic over-fired on public keys and id/type fields.
...
Deno.test("visibility (AIO-426): an exposed credential REFUSES to boot in dev", () => {
  // a news app Ugly #6: a warning is too soft for an unambiguous credential.
...
Deno.test("visibility: a deep-excluded container no longer warns (a crypto wallet 10/10)", () => {
...
Deno.test("visibility (a crypto wallet Ugly #7): all offending fields in ONE paste-ready message", () => {
  // We used to throw on the FIRST credential field, forcing a fix-one-rerun loop
  // (a crypto wallet listed six publicFields one crash at a time). Now one boot names them
