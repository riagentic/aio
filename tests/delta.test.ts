...
Deno.test('computeDelta: nested — only changed sub-key sent, not full slice', () => {
  const state1 = { counter: { count: 0 }, mdview: { html: '<h1>big</h1>', scrollY: 0, filePath: '/a.md' } }
  const state2 = { counter: { count: 0 }, mdview: { html: '<h1>big</h1>', scrollY: 42, filePath: '/a.md' } }
...
Deno.test('computeDelta: nested — multiple sub-keys changed in one feature', () => {
  ...
  const lastKeyJsons: Record<string, string> = { 'f.a': '1', 'f.b': '2', 'f.c': '3', 'f.d': '4', 'f.e': '5' }
  const result = _computeDelta(state2, state1, lastKeyJsons)
  assertEquals(result.kind, 'delta')
  const parsed = JSON.parse(result.msg)
...
Deno.test('computeDelta: nested — mixed flat and nested keys', () => {
  ...
  const lastKeyJsons: Record<string, string> = { version: '1', mode: '"dev"', 'counter.count': '0', extra: '"x"' }
  const result = _computeDelta(state2, state1, lastKeyJsons)
  assertEquals(result.kind, 'delta')
  const parsed = JSON.parse(result.msg)
...
function applyPatch(prev: Record<string, unknown> | null, msg: string): Record<string, unknown> {
...
Deno.test('applyPatch: nested patch shallow-merges into feature slice', () => {
  const prev = { counter: { count: 0 }, mdview: { html: '<h1>big</h1>', scrollY: 0 } }
  const result = applyPatch(prev, JSON.stringify({ $p: { mdview: { scrollY: 42 } } }))
...
Deno.test('applyPatch: proto-pollution blocked in $p keys', () => {
  const prev = { a: 1 }
  const result = applyPatch(prev, JSON.stringify({ $p: { __proto__: { polluted: true } } }))
...
Deno.test('applyPatch: proto-pollution blocked in $d', () => {
  ...
  const result = applyPatch(prev, JSON.stringify({ $p: {}, $d: ['__proto__'] }))
  assertEquals(typeof result.toString, 'function')
  assertEquals(result.a, 1)
})
...
Deno.test('applyPatch: proto-pollution blocked in nested $d', () => {
  ...
  const result = applyPatch(prev, JSON.stringify({ $p: { f: { $d: ['constructor', '__proto__'] } } }))
...
Deno.test('e2e: nested delta + patch round-trips correctly', () => {
  const state1 = { counter: { count: 0 }, mdview: { html: '<big>', scrollY: 0, file: '/a' } }
  const state2 = { counter: { count: 0 }, mdview: { html: '<big>', scrollY: 42, file: '/a' } }
