...
Deno.test('electronMainScriptUDS: contains socket path and IPC bridge', () => {
  const script = electronMainScriptUDS('http://localhost:8000', '/tmp/aio-test.sock', {})
...
Deno.test('electronMainScriptUDS: prod mode uses aio:// protocol', () => {
  const script = electronMainScriptUDS('http://localhost:8000', '/tmp/aio-test.sock', { baseDir: '/app/dist' })
...
Deno.test('electronMainScriptUDS: prod HTML includes CSS when hasCSS', () => {
  const script = electronMainScriptUDS('http://localhost:8000', '/tmp/aio-test.sock', { baseDir: '/app/dist', hasCSS: true })
  assertEquals(script.includes('style.css'), true)
})

Deno.test('electronMainScriptUDS: respects meta dimensions', () => {
  const script = electronMainScriptUDS('http://localhost:9000', '/tmp/aio-test.sock', { meta: { width: 1200, height: 900 } })
...
Deno.test('electronMainScriptUDS: default dimensions without meta', () => {
  const script = electronMainScriptUDS('http://localhost:8000', '/tmp/test.sock', {})
...
Deno.test('electronMainScriptUDS: sets app.name from title', () => {
  const script = electronMainScriptUDS('http://localhost:8000', '/tmp/test.sock', { title: 'My Dashboard' })
  assertEquals(script.includes('app.name = "my-dashboard"'), true)
})
...
Deno.test('electronMainScript: title with special chars', () => {
  const script = electronMainScript('http://localhost:3000', { title: "App's & <Title>" })
  assertEquals(script.includes('BrowserWindow'), true)
})
