...
| Bundle                                                      | gzip      | brotli |
| ----------------------------------------------------------- | --------- | ------ |
| aio: render a component                                     | 90 KB     | 76 KB  |
...
reactive in the browser without assembling five libraries, 90 KB gzipped is the
whole thing. Judge it against the second list, not the first.

Since alpha72 the server compresses and revalidates, so the counter app is 85 KB
...
| Bundle size                      | ⚠️    | 90 KB gzipped — the whole client, not a view layer (see §14) |
