...
| Bundle                                                      | gzip      | brotli |
| ----------------------------------------------------------- | --------- | ------ |
| aio: render a component                                     | 85 KB     | 74 KB  |
| aio: + one cell (the counter app)                           | 88 KB     | 76 KB  |
...
reactive in the browser without assembling five libraries, 88 KB gzipped is the
whole thing. Judge it against the second list, not the first.

Since alpha72 the server compresses and revalidates, so the counter app is 83 KB
on the wire and a 304 on every reload after that. That sits above the 76 KB in
...
| Bundle size                      | ⚠️    | 85 KB gzipped — the whole client, not a view layer (see §14) |
