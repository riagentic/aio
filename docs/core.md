...
> full explicit control — reach for it when methods can't express what you need.
...
The framework tracks the current machine state internally. Use `useFeature(counter).status` in the UI or `t.expect.status('idle')` in tests — never read `_status` directly from state.
...
Use `machine: false` for features that don't need state machine guards — all actions are always allowed.
...
**Always use `.type` or pass function directly** — never raw action type strings. Bound methods have `.type` as a static property:
...
The object form is the standard: one method per action key, `payload` typed from the action creator:
...
Effects are returned from the `execute` object's handlers. The reduce object cannot return effects — use `execute` for side effects.
...
Blocked dispatches log: `[te] dispatch('wallet:credit') blocked — add wallet to dispatchTo`

### `selectors` — derived state

Selectors receive the feature's own state slice — no need to navigate `state.featureName.field`:
...
const ws = feature('ws', {
  ...
  onInit(app) {
    ...
    const config = (app.getFullState?.()?.config as { url?: string } | undefined)?.url
...
When a feature has a multi-step async workflow (fetch → validate → save → notify), the standard reduce/execute pattern scatters the logic across actions, reducer cases, and effects. The `generators` key lets you write it top-to-bottom:
...
const wallet = feature('wallet', {
  ...
  reduce: {
    ...
    withdraw(state, payload) { state.balance -= payload.amount },
    sync() {},
  },
...
Reactive logic (deposit/withdraw) stays in `reduce`. Sequential workflows (sync) go in `generators`. Both work on the same state.
...
When a generator is triggered while a previous instance is still running, the old one is automatically cancelled. Use the `cancelOn` config key to declare cancellation triggers per generator:
...
- Bound methods directly (`healthCheck.stop`) — also works (`.type` is extracted)
- Full action type strings as last resort (`'healthCheck:stop'`) — lowercase format
...
Reactive features produce standard `FeatureDef` objects. They compose freely with all other `feature()` styles:

```ts
await aio.run({ features: [reactiveCounter, featureWallet, generatorCheckout] })
```

Other features can listen to reactive feature actions via foreign listeners, read via selectors, and coordinate via `call()`.
...
See [reactivity.md](reactivity.md) for async method patterns and [generators.md](generators.md) for the full generator API.
...
See [features.md](features.md) for the complete guide to feature interaction patterns.
...
After `aio.run()`, async methods return typed Promises. Import any feature and call its methods directly — no strings, no special syntax:
...
Everything goes through the store — dispatches a real action (`inventory:reserve`), observable in time-travel, intercepted by middleware. TypeScript infers the return type — no cast needed.

Every bound method has a `.type` property. Use it anywhere you'd otherwise write a raw action type string:
...
The callback form wraps the direct call with timeout/retry — the call inside goes through the store as normal.
...
- `call({ timeout, retries }, () => feature.method(args))` — when you need resilience

**vs `dispatchTo`:** `dispatchTo` is fire-and-forget from executor. Direct calling / `call()` awaits completion and returns values.

### `markAsync(fn)` — explicit async marking

Rarely needed. Standard `async function` syntax is auto-detected. Use `markAsync` only when minification strips constructor names (bundled JS, not standard Deno):
...
For Electron and local apps where the WS connection is near-instant, you can provide a fallback state to avoid the null check entirely:
...
TypeScript overload: when `fallback` is provided, the return type is `{ state: S }` (no null). Without fallback, it remains `{ state: S | null }`.
...
See [testing.md](testing.md) for the full testing reference — `testFeature()`, TestContext API, async testing, and property-based fuzzing.
