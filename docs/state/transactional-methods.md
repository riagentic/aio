# Spec: Transactional cell methods (wallet report #2)
...
Consequences a crypto wallet hit repeatedly (all real, all silent):
...
This is the a crypto wallet 2026-07-28 bug, verbatim: a balance `refresh()` guarded on a
...
- **Rollout:** ship phases 1–4 behind `transaction`, default off. Only after the
  ...
  it has soaked in a real app (a crypto wallet's `unlock`/`transfer` cells).
