...
| ----------------------------- | ---------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------- | --------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| `visible` (`ui`) `/ .exclude` | filters the broadcast; server code still reads everything                                | the hidden field is absent — a read throws, dev and prod alike. `scope: "client"` cells are exempt: their state never leaves the tab | filter applied on the main isolate as patches arrive — unchanged | not applied — `t.expect.state` is raw                           | applied to component reads; `ui.serverState()` is raw                         |
