...
Format: `AIO[N]` — one rule per line, compact and enforceable.

---

AIO1 All app logic MUST live in features created via feature('name', {...}) — no loose state, no ad-hoc logic outside features
...
AIO2 State MUST only be mutated inside methods (sync/async) or reduce handlers — never directly from outside
...
AIO3 Single entry point: aio.run({ appId, features: [...] }) — no manual store creation, no manual server setup
...
AIO4 UI components MUST access state via useFeature(ref) or useAio() hooks — never import or read state directly
...
AIO5 Cross-feature communication MUST use direct method calls or listensTo — never raw dispatch with string action types
...
AIO6 All bound feature methods return Promise — use await for synchronization outside of sync methods
...
AIO7 Sync methods (reducers) MUST NOT contain side effects — only state mutations and fire-and-forget dispatches
