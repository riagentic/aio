...
For the docs index, see [manual.md](manual.md). For reactive features, see
[reactivity.md](reactivity.md). For generator-based workflows, see
...
Async test functions are supported — `await t.settle()` runs effects + waits for
async to complete:
...
The string form `call('feature', 'method', ...)` was removed in v0.8 — use
direct import and calling for type safety.
