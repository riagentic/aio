...
| `--profile=X`         | Run from profile `X`'s data home — a name (`~/.<appId>-X`, its own lock, socket and Electron profile) or a path; also `AIO_PROFILE`. Refused under `profiles: false`. See [profiles](../clients/app-manager.md#profiles-several-copies-of-one-app) |
| `--home=DIR`          | Run from data home `DIR` — the path form of `--profile` (env: `AIO_PROFILE` with a path). Giving both, naming two folders, is refused                                                                                                              |
