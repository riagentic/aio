...
deno task bench:check    # run bench, then gate against bench-baselines.json
...
`bench-baselines.json` (committed) holds one **floor per metric**: `maxMedian` —
the maximum acceptable median. `scripts/check-bench.ts` compares each measured
median against its floor and exits 1 naming the metric, measured value, and
floor on any violation. p95 is recorded for context but not gated (too noisy for
a shared machine).
