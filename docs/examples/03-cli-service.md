...
A start-to-finish walkthrough building a task queue service. No Electron, no browser. Just a headless server you control via `am` and `connectCli`.

## What we're building

A job queue that accepts tasks via CLI or web, processes them in order, and reports status. You will wire up:
...
export const queue = feature('queue', {
  ...
  methods: {
    enqueue(s, task: string) {
      s.jobs.push({ id: crypto.randomUUID().slice(0, 8), task, status: 'pending', createdAt: Date.now() })
    },
    complete(s, jobId: string) {
      const job = s.jobs.find(j => j.id === jobId)
      if (!job || job.status !== 'processing') return
      job.status = 'done'; s.processing = null; s.completed += 1
    },
    fail(s, jobId: string, error: string) {
      const job = s.jobs.find(j => j.id === jobId)
      if (!job || job.status !== 'processing') return
      job.status = 'failed'; job.error = error; s.processing = null; s.failed += 1
    },
    async process(s) {
...
The `process` method is async -- each state mutation after an `await` goes through the dispatch loop, so everything persists and shows up in time-travel automatically.
...
The scheduler dispatches `queue:process` every second. If nothing is pending or a job is already running, the method returns early. No manual polling loop needed.
...
type QueueState = {
  queue: { jobs: Job[]; processing: string | null; completed: number; failed: number }
}
...
cli.subscribe(s => {
  ...
  console.log(`[queue] pending=${pending} processing=${q.processing ?? 'none'} done=${q.completed} failed=${q.failed}`)
})
...
`connectCli` uses the same WebSocket delta protocol as the browser. It auto-reconnects with exponential backoff and queues actions sent before the connection opens.

> **Payload format:** Methods-style features expect `{ args: [...] }` as the payload. `queue.enqueue(task)` becomes `{ type: 'queue:enqueue', payload: { args: [task] } }`.
...
> `am` output auto-detects the terminal. When piped, it emits JSON -- pipe to `jq` for scripting.
...
This produces `task-queue` (binary) and `task-queue.service` (systemd unit) with `ExecStart` set to `--headless --port=3000`. Install:
...
For remote access, add `--remote` to the compile command. That puts `--expose` in ExecStart, which binds `0.0.0.0`, enables auto-HTTPS, and generates an auth token (printed in the journal on first boot).
...
Same API, same code -- just a different URL and a token. `am` works remotely too: `deno task am state --port=3000 --app=task-queue`.
...
That covers the full service lifecycle: define features, schedule work, control via CLI and `am`, compile to a binary, deploy with systemd, connect remotely. No React, no Electron, no browser -- just aio doing what it does with state, on a server.
