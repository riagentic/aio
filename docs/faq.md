...
sequential workflows with branching, use [generators](generators.md).
...
React is the only maintained adapter. The `client` API is stable and supported —
adapters built on it are your responsibility.
...
[howto.md](howto.md#pattern-form-state-with-validation) for the full pattern.
...
clustering, or a custom CRDT layer. See [scaling.md](scaling.md) for capacity
guidance.
