...
async function patchDenoJson(projectDir: string, patch: (config: DenoJsonConfig) => void): Promise<boolean> {
  const path = join(projectDir, 'deno.json')
  let text: string
  try { text = await Deno.readTextFile(path) } catch {
    try { text = await Deno.readTextFile(join(projectDir, 'deno.jsonc')) } catch { return false }
  }
  try {
...
export function fixAddNodeModulesDir(projectDir: string): Promise<boolean> {
  return patchDenoJson(projectDir, (c) => { if (!c.nodeModulesDir) c.nodeModulesDir = 'auto' })
}
...
export function fixAddTypesReact(projectDir: string): Promise<boolean> {
  return patchDenoJson(projectDir, (c) => {
    ...
    if (!c.imports['@types/react']) c.imports['@types/react'] = 'npm:@types/react@^18'
  })
}
...
export function fixRemoveImportReact(filePath: string): () => Promise<boolean> {
  return () => removeLine(filePath, /^\s*import\s+React[\s,{].*from\s+['"]react['"]/)
}

/** Remove `import { createRoot } from 'react-dom/client'` — framework handles mounting */
export function fixRemoveCreateRootImport(filePath: string): () => Promise<boolean> {
  return () => removeLine(filePath, /^\s*import\s+\{[^}]*createRoot[^}]*\}\s+from\s+['"]react-dom\/client['"]/)
}
