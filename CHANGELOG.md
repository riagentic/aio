...
Five field reports (a trading platform, a crypto wallet ×2, a prediction-market app, a news app) worked end to end. The theme:
...
- **Direct cell access now reliably subscribes to server deltas** and cell signals
  are no longer orphaned across re-renders (a crypto wallet CRITICAL) — with a real e2e
...
- **14 verified bugs** from the GLM-5.2 multi-aspect audit, and three fail-loud
  gaps from that wallet's report (16e, 16f-b, 17b), each pinned by regression tests.
