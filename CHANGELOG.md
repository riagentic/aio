...
lifecycle correctness, and a public component test harness (a desktop tool session 1–2
feedback).
...
- **Backoff on rate-limit** — worked self-scheduling `after`-chain pattern for
  ...
  `schedule.every` and static schedules (a crypto wallet P2)
- **Keyed map with default** — declare-once accessor pattern for
  `Record<string, T>` cell reads in JSX, no sprinkled `?? 0` guards (a crypto wallet P3)
