...
### From the field (a crypto wallet, day one on cell workers)
...
The dream-list release: a real wallet (a crypto wallet, ~650 tests) drove its whole
...
**Field-report P1s (machine, a news app R4).** The `Deno is not defined` blank-screen
...
being silently dropped in prod (a news app R4). `ui.surface()` staleness fixed at the
...
Five field reports (a trading platform, a crypto wallet ×2, a prediction-market app, a news app) worked end to end. The
...
- **Direct cell access now reliably subscribes to server deltas** and cell
  signals are no longer orphaned across re-renders (a crypto wallet CRITICAL) — with a
...
- **14 verified bugs** from the GLM-5.2 multi-aspect audit, and three fail-loud
  gaps from that wallet's report (16e, 16f-b, 17b), each pinned by regression
  tests.
...
Every open item from all three field reports (a crypto wallet, a trading app, mdview) closed —
...
- **`bootCells` + virtual-clock schedules — every effect is now testable.**
  ...
  that blocked "test every use case" in that wallet's report — countersigned 10/10.
...
- **Conditional element bindings froze inside `<form>` (a crypto wallet 2026-07-16d).**
...
- **SVG camelCase attrs render.** `stopColor` → `stop-color` (and the common
  camelCase set) — gradients no longer render black (trading-app report Ugly #2).
- **Async multi-await write loss locked.** Writes after any `await` are
  ...
  loudly: every `await` commits + renders (trading-app report Ugly #1).
- **Dev failures got loud (a trading app's thesis: no quiet failures).** Discovery bind
...
## 1.0.0-alpha16 — deep-audit cleanup + field-report fixes (mdview, a crypto wallet)

A full per-file audit (no correctness bugs found) plus the cleanup it turned up,
and every open item from the mdview and a field reports. Non-breaking:
