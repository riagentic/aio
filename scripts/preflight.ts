...
const BIN = `${Deno.env.get("DENO_INSTALL_ROOT") ?? `${Deno.env.get("HOME")}/.deno`}/bin/${AM}`;
...
const steps: { name: string; run: () => Promise<string> }[] = [
  ...
  },
  {
    ...
    run: async () => {
      ...
      const r = await sh("deno", ["publish", "--dry-run", "--allow-dirty"], REPO);
      if (r.code !== 0) fail(`publish dry-run failed:\n${tail(r.err || r.out)}`);
  ...
  },
  {
    ...
    run: async () => {
      const r = await sh("deno", [
        "install", "-gAfr", "-n", AM, "--config", `${REPO}/deno.json`, `${REPO}/src/am.ts`,
      ]);
      if (r.code !== 0) fail(`deno install: ${tail(r.err)}`);
      const v = await sh(BIN, ["version", "--json"]);
      if (v.code !== 0) fail(`${AM} version: ${tail(v.err)}`);
      const version = (JSON.parse(v.out.trim() || "{}") as { version?: string }).version;
  ...
  },
  {
    ...
    run: async () => {
      ...
      const r = await sh(BIN, ["create", "app", `--mirror=${REPO}`, "--json"], parent);
      if (r.code !== 0) fail(`am create: ${tail(r.err || r.out)}`);
      appDir = `${parent}/app`;
      const want = ["deno.json", "src/app.ts", "src/cell.ts", "src/App.tsx", "src/cell.test.ts"];
      for (const f of want) {
        try {
  ...
  },
  {
    ...
    run: async () => {
      const r = await sh("deno", ["check", "src/app.ts", "src/App.tsx", "src/cell.test.ts"], appDir);
...
try {
  for (const [i, step] of steps.entries()) {
    ...
    Deno.stdout.writeSync(
      new TextEncoder().encode(`  ${c.dim(`${i + 1}/${steps.length}`)} ${step.name} … `),
    );
    try {
...
async function pushNote(): Promise<void> {
  try {
    const branch = (await sh("git", ["rev-parse", "--abbrev-ref", "HEAD"], REPO)).out.trim();
    const dirty = (await sh("git", ["status", "--porcelain"], REPO)).out.trim().length > 0;
    const ahead = (await sh("git", ["rev-list", "--count", `origin/${branch}..HEAD`], REPO)).out.trim();
    ...
    if (pending.length) {
      ...
      console.log(c.dim(`    \`curl | sh\` clones origin/${branch} — commit + push to make these live.`));
    } else {
      console.log(c.grn(`  ✓ origin/${branch} is up to date — \`curl | sh\` clones exactly this.`));
    }
  } catch {
