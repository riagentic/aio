...
async function main(): Promise<void> {
  ...
  for (const [v, locs] of [...seen.entries()].sort()) {
    ...
    console.log(`  ${status} v${v}  (${locs.length} occurrence${locs.length > 1 ? 's' : ''})`)
  }
...
async function detectVersion(): Promise<string | undefined> {
  try {
    const changelog = await Deno.readTextFile(new URL('../docs/changelog.md', import.meta.url).pathname)
