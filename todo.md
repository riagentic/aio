...
- **Cells-aware dev watch** (trading-app report Bad #3) — a cell edit restarts the app
  instead of warning about it (alpha35, `docs/build/dev-mode.md`).
- **`aio/server` import split** (a crypto wallet) — server-only symbols live behind
...
- [ ] **Profile integrity.** `quick_check` at boot → restore from a rolling
      ...
      that persists user data eventually wants; a crypto wallet wrote its own. Would be
...
- **`listensTo` low-latency fan-out queue** (a trading app) — one app, one perf profile;
...
`feedback/*.md` (glm-audit, a news app, machine, mdview, a trading app, a prediction-market app, a crypto wallet,
a trading platform) were retired once every item was closed, refused with a reason, or listed
here. They live on in git history; these are the only pieces that outlived them:

- [ ] **An end-to-end CRUD example** (a prediction-market app Bad#10, the one item they rated
      _high_) — `examples/` is counter, todo, and the per-target smoke fixtures.
      "The pieces are documented; the integration story isn't."
- [ ] **`testCell<S>` inference from the cell** (a trading platform B8; M7 is the same root —
      type-safe cell config). Deliberately not rushed: it changes inference in a
      helper every test file uses.
- [ ] **Parameterized selectors** `byId(id)` (a prediction-market app, low) — plain selectors
      work in the browser since alpha23; the parameterized form is the gap.
      `items.find(...)` inline works.
- [ ] **Cross-runtime `seed()` hook** (a trading platform M6, low) — convenience; `onRestore`
      covers the server side.
- [ ] **`useLocal` tuple form under `noUncheckedIndexedAccess`** (a prediction-market app, low)
      — the object form is the documented workaround. Verify, then close.
- [ ] **`am instances`** as the "what's running" command (a prediction-market app, low) —
      `am discover` + amui cover most of it.
- [ ] **`aio ship` auto-update client** (wallet report #9, deferred as large) — the
      signing foundation shipped; the client half is the remaining piece.
- [ ] **Headless-electron e2e** (wallet report #5) — parked: headless Electron stalls
