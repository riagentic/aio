...
- **`am restart` drops the argv the app was started with** (composer §5, a crypto wallet
...
- `am instances` should print the DATA path beside each row (wallet report §20): three
...
- `am heap <app>` (trading-app report §9.4). A console peaked at 31.8 GB and restarted 16
...
- **An error boundary** (wallet report §22.1). A component that throws during render
...
- **`onPersist(state)` / `persist: { include, transform }`** (cc §8.4). aio lets
  ...
  sync by hand.
- **A `budgets` block** (trading-app report §9.3).
...
- **`aio/server-only` and `aio/client-only` marker modules** (trading-app report §9.1). The
...
- **`testUI`'s window is not `globalThis`** (wallet report §19.2). A listener attached
...
- **Geometry in tests** (wallet report §19.1, §22.2; anathomy §10.1). happy-dom
...
- **`t=` is a handle, not an attribute, and does not appear in `ui.html()`**
  (wallet report §19.3). One doc line.
...
- **Mount-time rehydration is racy and silent** (llama.master §11). Measured at
  ...
  affected test.
- **A trace artifact on failure** (trading-app report §9.5) — dump the AIR tree and the last
...
- **A CSP that can drop `base-uri`, and a nonce for the inline shell** (newjob
  §3, wallet report §11). `base-uri` is described as one of "the directives that cannot
...
- **A `patch` watcher signal** (newjob §8.3, vidtune §12.2, watcher §8.2, a crypto wallet
  ...
  (newjob), 760 MB of GPU weights (watcher), and a wallet's unlock (a crypto wallet). AIR
...
- **A component profiler / re-render log** (llama.master §16, wallet report §22.3). One
...
- **A bundle treemap / `--analyze`** (newjob §8.9, wallet report §22.7).
...
- **A version pin makes `.katana/_aio.md`'s own instruction unreachable**
  ...
  inside the versioned directory, so an upgrade deletes it (trading-app report §7).
...
- **The 100-msg/sec WS budget is tripped by a legitimate burst** (wallet report §10). A
...
- A `worker: true` hint when a cell's tick exceeds a frame budget (trading-app report §3),
  and the module-graph checker's output in the default build summary.
- A documented known-good Electron switch set for headless and VM hosts (a trading app
...
- `feedback/` lives inside the versioned directory, so an upgrade deletes it
  (trading-app report §7) — the tooling costs the project exactly the reports it wants.
- A CI recipe for an app built on aio (newjob §8.9).
- `deno task am migrate --from=alpha76` for known renames (wallet report §22.7).
- A named way to keep a cell out of the dev action journal, distinct from
  `persist` (trading-app report §7, and see `refused.md`).
