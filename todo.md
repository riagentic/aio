...
- **Append-delta broadcasts for growing values** (ask 5). A streamed reply is a
  ...
  same granularity debt `a trading app` and `a crypto wallet` pay; three consumers now.
...
## From the a field report (money software, alpha59 → triaged at alpha61)
...
- **db:/persist granularity** (RIS-5a/6, and a trading app before them): object-shaped
