...
- [ ] **A6 — Field reports for untested app types.** Covered: TUI/desktop
      (a crypto wallet), AIR/canvas (a desktop tool). Missing: `android`, `remote-*`,
...
- [ ] **B5 — Security pass.** WS auth, secret-state exclusion from persist/ui
      (a crypto wallet's pattern → documented invariant), `scope:"client"` boundary.
...
- Keep the field-report ritual; pin the a crypto wallet/a desktop tool keep-lists as tests where
  possible.
