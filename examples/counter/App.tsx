  ...
  return (
    <div style={{ padding: '3rem', fontFamily: 'system-ui, sans-serif', textAlign: 'center' }}>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center' }}>
        <button type="button" onClick={() => send.decrement()} style={btn}>−</button>
        <button type="button" onClick={() => send.reset()} style={btn}>Reset</button>
        <button type="button" onClick={() => send.increment()} style={btn}>+</button>
      </div>
    </div>
  )
}
